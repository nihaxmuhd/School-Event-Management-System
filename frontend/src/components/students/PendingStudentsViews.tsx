...
export const PendingStudentsView: React.FC<PendingStudentsViewProps> = ({
  ...
  // Pending = students not assigned/registered to ANY event
  const pendingStudents = students.filter(s => s.registeredEventIds.length === 0);
  ...
  const pendingByHouse = houses.map(h => ({
    house: h,
    count: pendingStudents.filter(s => s.houseId === h.id).length
  }));

  const assignedPct = students.length > 0
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white p-5 rounded-2xl border border-slate-200/80 shadow-xs">
        <div>
          <h3 className="text-base font-bold text-slate-900 flex items-center gap-2">
            <UserX className="w-5 h-5 text-amber-500" /> Pending Students
          </h3>
          <p className="text-xs text-slate-500">
            {pendingStudents.length} students not yet assigned to any event • {assignedPct}% of school assigned
          </p>
        </div>

        <div className="flex items-center gap-2">
          <button
            onClick={() => { downloadExcel('pending_students', 'Pending Students', buildRows()); onShowToast('Excel Downloaded', 'Exported pending students to Excel'); }}
          ...
          <button
            onClick={() => { downloadPDF('Pending Students Report', 'Students not yet registered for any event', buildRows()); onShowToast('PDF Ready', 'Opening print dialog for PDF export'); }}
            className="px-3.5 py-2 bg-slate-100 hover:bg-slate-200 text-slate-700 font-semibold text-xs rounded-xl flex items-center gap-1.5 transition-colors cursor-pointer"
          >
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
        <div className="bg-white rounded-2xl border border-slate-200/80 shadow-xs p-4">
          ...
          <p className="text-2xl font-black text-amber-600 mt-1">{pendingStudents.length}</p>
        </div>
        {pendingByHouse.map(({ house, count }) => (
          <div key={house.id} className="bg-white rounded-2xl border border-slate-200/80 shadow-xs p-4">
            <div className="flex items-center gap-1.5">
              <span className="w-2.5 h-2.5 rounded-full" style={{ backgroundColor: house.color }} />
              <p className="text-[10px] font-bold uppercase tracking-wider text-slate-400 truncate">{house.name.replace(' House', '')}</p>
            </div>
              ...
              {filtered.length === 0 ? (
                <tr>
                  <td colSpan={7} className="text-center py-12 text-slate-400">
                    ...
                    All students in this view have been assigned to events. 🎉
                  </td>
                </tr>
              ) : (
                filtered.map(st => {
                  ...
                  return (
                    <tr key={st.id} className="hover:bg-slate-50/80 transition-colors">
                      ...
                      <td className="py-3.5 px-4">
                        <span className={`px-2 py-0.5 rounded text-[10px] font-bold ${st.gender === 'Male' ? 'bg-blue-50 text-blue-700' : 'bg-pink-50 text-pink-700'}`}>
                          {st.gender}
                        </span>
                      </td>
                      <td className="py-3.5 px-4">
